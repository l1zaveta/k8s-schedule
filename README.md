...
Приложение представляет собой REST API на базе FastAPI, упакованное в Docker-контейнер
и развёрнутое в кластере Kubernetes. 
...
- Развёрнут локальный кластер Kubernetes с помощью minikube
- Создан и задеплоен Docker-образ приложения ИС «Расписание»
- Настроено горизонтальное автомасштабирование: при нагрузке CPU >50%
  количество подов автоматически увеличивается с 2 до 5
- Развёрнута система мониторинга Prometheus + Grafana с дашбордом
  для наблюдения за состоянием кластера в реальном времени

Использование Kubernetes позволяет обеспечить отказоустойчивость
и масштабируемость системы управления расписанием без ручного
вмешательства при изменении нагрузки.
